...
export const createCommands = (): CommandDefinition[] => [
  ...
  command(
    ...
    "Call tool mario_devx_add with feature=\"$ARGUMENTS\". If the result starts with 'Feature interview', extract the interview question text (exclude the leading 'Feature interview (x/3)' line when present). If the question is boolean (for example starts with words like 'Does', 'Is', 'Should', 'Can', 'Will' or requests yes/no), ask the question tool (header: 'Feature Add') with options exactly: 'Yes' and 'No', with custom input enabled. Otherwise ALWAYS provide exactly 3 suggested answers. Suggestions must be direct answer candidates to THAT question only, concrete feature content (not process/meta commentary), short and scannable (max 18 words each, single sentence/phrase, no bullet formatting, no semicolon chains, no long enumerations). Never include words like: question, option, recommended, hardcoded, generated, LLM, session, single-choice, multi-choice, free-text. Ask the question tool (header: 'Feature Add') with options in order: suggestion 1, suggestion 2, suggestion 3; keep custom input enabled so 'Type your own answer' is available. CRITICAL: after showing the question tool, WAIT for user interaction; do not call mario_devx_add again unless you received a concrete answer from the question tool (selected suggestion label or custom text). If the selected value is exactly 'Type your own answer', treat it as NO answer yet and reply: 'Type your answer in your own words and submit.' If there is no concrete answer yet, return the current interview question text unchanged. Otherwise call mario_devx_add again with feature=<chosen answer>. Return only the final tool output.",
  ),
  command(
    "replan",
